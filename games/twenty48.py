...
from renderers.twenty48_renderer import WIDTH, HEIGHT, GRID_SIZE
...
class Twenty48View(arcade.View):
    ...
    def _create_texts(self):
        ...
        from renderers.twenty48_renderer import (
            GRID_LEFT, GRID_BOTTOM, GRID_PIXEL, CELL_GAP,
            CELL_SIZE as R_CELL_SIZE,
        )
        self.txt_title = arcade.Text(
            "2048", WIDTH / 2, HEIGHT - 30, (119, 110, 101),
            font_size=36, anchor_x="center", anchor_y="center", bold=True,
        )
        self.txt_score = arcade.Text(
            "", WIDTH / 2, HEIGHT - 65, (119, 110, 101),
            font_size=20, anchor_x="center", anchor_y="center",
        )
        # Cell value texts - one per grid cell
        self.txt_cells = {}
        for r in range(GRID_SIZE):
            for c in range(GRID_SIZE):
                x = GRID_LEFT + CELL_GAP + c * (R_CELL_SIZE + CELL_GAP) + R_CELL_SIZE / 2
                y = GRID_BOTTOM + GRID_PIXEL - (CELL_GAP + r * (R_CELL_SIZE + CELL_GAP) + R_CELL_SIZE / 2)
                t = arcade.Text(
                    "", x, y, (119, 110, 101),
                    font_size=40, anchor_x="center", anchor_y="center", bold=True,
                )
                self.txt_cells[(r, c)] = t
        # Overlay texts
        self.txt_overlay_title = arcade.Text(
            "", WIDTH / 2, HEIGHT / 2 + 20, (119, 110, 101),
            font_size=52, anchor_x="center", anchor_y="center", bold=True,
        )
        self.txt_overlay_subtitle = arcade.Text(
            "", WIDTH / 2, HEIGHT / 2 - 30, (119, 110, 101),
            font_size=20, anchor_x="center", anchor_y="center",
        )
    ...
    class _DummyMenuView(arcade.View):
        ...
        def __init__(self):
            ...
            self.txt_menu = arcade.Text(
                "Menu (press Enter for 2048)", WIDTH / 2, HEIGHT / 2,
